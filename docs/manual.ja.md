...
ウィンドウは **[削除]** と **[統合]** の 2 つのタブに分かれています。
...
ツールバーの下に **[削除]** と **[統合]** のタブがあります。開いたときは削除タブです。**Ctrl+Tab** でも切り替わります。

- **「すべてを選択」「選択解除」は、前面に出ているタブにだけ効きます。** 見えていないタブのオブジェクトが勝手に対象になることはありません。
- **「削除して保存」はどちらかにチェックがあれば押せます。** 1 回の保存で両方が処理されます。
- ステータスバーの件数も、前面のタブのものです。
...
| すべてを選択 | 前面のタブで、表示中のオブジェクトをすべてチェックします |
| 選択解除 | 前面のタブのチェックをすべて外します |
...
> **先に削除すると統合できません。** 焼き込む材料が無くなるからです。ただし 1 回の保存の中では**統合 → 削除**の順に処理されるので、両方のタブでチェックしてから保存すれば問題ありません。
...
- 共有された描画部品（Form XObject）の中身は対象外です（削除タブの「削除不可」と同じ理由）。

### 使い方

1. **[統合] タブ**を開きます。
2. 左のツリーが **文書 → ページ → 統合単位 → オブジェクト**の 4 階層になっています。「統合単位」が、1 枚の画像に焼き込まれる 1 か所です。
3. **チェックボックスは統合単位とオブジェクトにあります。** 単位にチェックを付けると配下のオブジェクトが全部付き、個別に選ぶこともできます。文書とページにチェックボックスはありません（そこは並べるためだけの階層です）。
4. **チェックしたオブジェクトだけ**が焼き込まれ、画像になる範囲も**チェックしたものの外接矩形**です。たとえばテキストだけにチェックを付ければ、下の画像はそのまま残り、その上に生成された画像が重なります。
5. 右のプレビューで、選んでいるノードの場所が確認できます。**その場所だけ色が残り、それ以外は暗く表示されます。**
   - 文書を選ぶと 1 ページ目、ページを選ぶとそのページの統合単位すべて、統合単位を選ぶとその中のオブジェクトすべて、オブジェクトを選ぶとそれ 1 つが示されます。
6. **保存は削除と同じ**です（ツールバーの「削除して保存」、または **ファイル → 選択画像を削除して保存…**）。統合だけでも保存できます。

保存が終わると、統合タブの一覧は**そのまま残り、チェックだけが外れます。** 一覧が説明しているのは元の PDF で、元の PDF は変わっていないからです（削除タブは逆に、取り除いた行が一覧から消えます）。

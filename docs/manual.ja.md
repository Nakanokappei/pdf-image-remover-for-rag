...
| すること | 選んだオブジェクトを取り除く | 重なっているオブジェクトを 1 枚の画像に焼き込む |
...
統合は、その重なっている場所を**そのままの見た目の 1 枚の画像に焼き込みます。** ページの見え方は変わりません。変わるのは中身で、**その場所のテキストはテキストでなくなります**。RAG のパイプラインが読むのはテキスト層なので、そこから消えるわけです。
...
> **先に削除すると統合できません。** 焼き込む材料が無くなるからです。ただし 1 回の保存の中では**統合 → 削除**の順に処理されるので、左右の両方にチェックを付けてから保存すれば問題ありません。
...
- **テキスト同士の重なりは対象外です。** 焼いても文字が絵になるだけで、得るものがありません。
...
4. **チェックしたオブジェクトだけ**が焼き込まれ、画像になる範囲も**チェックしたものの外接矩形**です。たとえばテキストだけにチェックを付ければ、下の画像はそのまま残り、その上に生成された画像が重なります。
...
- **焼き込んだ場所の文字は、選択もコピーもできなくなります。** それがこの機能の目的です。もとの PDF は変わらないので、必要なら開き直せます。

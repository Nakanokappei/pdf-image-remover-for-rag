...
///
/// It decides whether a picture that is already small enough for the screen is
/// worth writing again. Re-encoding one that was saved BELOW the ceiling costs
/// detail and can cost bytes as well — a photo saved at 60 and written out at
/// 85 grows, and the detail it lost is not coming back. So an image nobody has
/// to resize is only re-encoded when it is above the ceiling, where dropping to
/// it is the saving that was asked for.
///
/// A resized image is a different case and does not consult this: its pixels
/// are being redrawn anyway, and they are written at the ceiling whatever the
/// original was.
///
